...
namespace sin_GraphCore
{
    public partial class sin_GraphCore : UserControl
    {
        ...
        private void LoadDefaultValues()
        {
            ...
            else Values = new float[plChart.Size.Width / 2];
                ...
                for (int i = Values.Length - CurrentNumberOfValues; i < Values.Length; i++)
                {
                    if (Values[i] >= 0)
                    {
                        if (IntCmp(Values[i], Values[i - 1]) > 0)
                        {
                            ...
                            g.DrawLine(AbovePen, (Values.Length - i) * 2 - 1
        ...
        public void AddValue(float val)
        {
            ...
            {
                if ((val * ValueMultiplier > Maximum) || (val * ValueMultiplier < Minimum)) return;
